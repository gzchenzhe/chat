...
## 11. 2026-07-16 气泡弹性宽度更新

- 文字气泡保留 255pt 标准宽度，并根据实际字体测量与换行结果，最多向对侧借用 5pt；右侧消息固定右边缘向左扩展，左侧消息固定左边缘向右扩展。
- 短消息继续按内容收缩；图片、语音等非文字消息不使用本次弹性规则。字体加载完成后会重新测量，预览与 PNG 导出共用同一 DOM 宽度。
- 新增镜像边界自动回归；6 项 Playwright E2E 全部通过。浏览器实测 PNG 仍为 1125×2436，`git diff --check` 通过。
- 已知既有检查问题：静态检查中“聊天标题必须无额外字距”仍失败，因为当前已提交样式为 `letter-spacing: 1px`，而检查脚本要求 `0`；与本次气泡改动无关。

## 12. 2026-07-16 iOS 中文断行一致性修复

- 中文、全角标点改用固定 16pt 逻辑字宽断行，气泡宽度和每行内容不再由各端系统字体的实际字宽决定；桌面端与 iOS 共用相同的 255–260pt 气泡布局结果。
- iOS WebKit 禁用文字自动放大，并在字体加载、消息更新、页面切换、窗口变化和导出前，对每行实际渲染宽度做一次校准；只压缩超出基准的行，不改变桌面端正常结果。
- 新增 iOS 字宽放大模拟回归，并核对预览及最终 1125×2436 PNG；7 项 Playwright E2E 全部通过，`git diff --check` 与 JavaScript 语法检查通过。
- 既有静态检查仍只有“聊天标题必须无额外字距”一项失败，与本次断行修复无关；真实 iPhone Safari 仍建议在缓存更新到 v34 后复测一次。
